...
const fullNamesArr = peeps.map(({ name: { first, last } }) => `${first} ${last}`);
...
//const loveComments = Object.values(comments).filter(c=>c.text.toLowerCase().includes('love')).map(c=>c.text);
...
const sortedCommentsArray = Object.values(comments).filter(i => i.hasOwnProperty('rating')).sort((a, b) => a.rating - b.rating)
...
const commentObj = Object.keys(comments).reduce((a, c) => ({ ...a, [c]: comments[c].text }), {});
...
const ratingAvg = Object.values(comments).reduce((a, { rating }) => a += rating || 0, 0) / Object.values(comments).filter(c => c.rating).length
...
const groupedPeepComments = peeps.reduce((a, { name: { first, last }, id }) => {
    a[`${first.toLocaleLowerCase()}${last}`] = Object.values(comments).filter(i => id === i.userId);
    return { ...a }
}, {})
