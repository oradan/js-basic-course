...
console.log(
  "3. Return/save in a variable  “true” if everyone is older than 24 and save in a constant."
);
...
const youngPeeps = peepsLessThan30.map(
  ({ name: { first, last }, age, id }) => `${first} ${last} ${age} ${id}`
);
...
const sortedPeeps = peepsSorted.map(
  ({ name: { first, last }, age, id }) => `${first} ${last} ${age} ${id}`
); //destructurare
...
console.log(
  "1. List all the comments text  in an array of strings called “commentsTextArray” "
);
...
const ratings = Object.values(comments)
  ...
  .map((comment) => comment.rating); //returneaza ratingurile fara cele care nu au rating
const averageRating =
  ratings.reduce((acc, currval) => acc + currval, 0) / ratings.length;
